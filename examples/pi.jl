...
T_EXTRA       = 2580   # extra time for simulation
TURBULENCE    = true
...
demand_data = [t < 240 ? 1.0 : max(0.7, 1.0 - (t - 240) * 0.0005) for t in time_vector]
...
mse = sum((rel_power .- demand_data[1:length(rel_power)]).^2) / length(rel_power)
